# Gestão e metodologias (Scrum, Kanban, Six Sigma, PMI/PMBOK)

Fundamentos e práticas de frameworks e corpos de conhecimento usados em gestão de projetos, produtos e melhoria de processos. Cada capítulo é um artigo autocontido (conceitos, diagramas, referências e exemplos de código ilustrativos).

---

## Índice

1. [Scrum — framework ágil para produtos complexos](./01-scrum.md)
2. [Kanban — fluxo, WIP e melhoria contínua](./02-kanban.md)
3. [Six Sigma — redução de variação e melhoria de processos](./03-six-sigma.md)
4. [PMI e PMBOK — gestão de projetos e performance domains](./04-pmi-pmbok.md)

---

## Referências gerais

- [Scrum Guide](https://scrumguides.org/scrum-guide.html)
- [Kanban Guide for Scrum Teams](https://scrum.org/resources/kanban-guide-scrum-teams)
- [ASQ — Six Sigma](https://asq.org/quality-resources/six-sigma)
- [PMI — Project Management Institute](https://www.pmi.org/)
